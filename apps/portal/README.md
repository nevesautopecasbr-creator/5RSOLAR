...
- **Proposta sob demanda**: `POST /api/projects/[id]/proposal` — gera PDF para um projeto existente; body opcional `{ "sendWhatsApp": true, "phone": "5511999999999", "createSigning": true, "clientSigner": { "name", "email" }, "companySigner": { "name", "email" } }`.
- **Assinatura digital (Clicksign)**: após gerar a proposta, o sistema pode criar um envelope na Clicksign com dois signatários (cliente e empresa). Env: `CLICKSIGN_ACCESS_TOKEN`, `COMPANY_SIGNER_NAME`, `COMPANY_SIGNER_EMAIL`; opcional `CLICKSIGN_API_BASE` (padrão sandbox). No webhook Monday, se houver e-mail do lead e company signer configurado, a solicitação de assinatura é criada automaticamente.
- **Webhook de assinatura**: `POST /api/webhooks/signature` — configurar na Clicksign como URL de notificação. Quando o documento for assinado por ambas as partes (evento closed/envelope_closed), o sistema atualiza o projeto para `pipelineStatus = 'ativo'`, move o arquivo de `proposals/` para `contracts/` no Storage e atualiza o Document para tipo `contrato`.
