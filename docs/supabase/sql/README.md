...
4. Repita para `02-refresh-tokens.sql`, depois `03-project-budget.sql`, e assim por diante até `17-leads-table.sql`.
...
O arquivo **`00-schema-completo.sql`** contém todo o schema na ordem correta (equivalente a rodar 01 a 11 em sequência). Cole no SQL Editor do Supabase e execute **uma vez** em um banco vazio. **As migrações 12 a 17 (5R backlog, Document, SolarUnit, RLS, SigningRequest, Lead)** devem ser aplicadas em seguida, nessa ordem.
